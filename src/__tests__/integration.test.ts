...
describe('Integration Tests', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should handle action failure', async () => { const mockHands={isConnected:()=>true,findElementByText:vi.fn().mockResolvedValue({id:'button_1',text:'Click',bounds:{x:100,y:200,width:200,height:50}}),tap:vi.fn().mockResolvedValue({success:false,error:'Tap failed'})}; vi.spyOn(handsManager,'isConnected').mockResolvedValue(true); vi.spyOn(handsManager,'getHands').mockReturnValue(mockHands as any); const result=await toolRegistry.executeTool('tap_element',{elementText:'Click'}); expect(result.success).toBe(false); expect(result.error).toContain('Tap failed'); });
