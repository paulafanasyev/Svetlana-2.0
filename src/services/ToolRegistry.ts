...
export type ToolBackend = 'LOCAL_FREE' | 'USER_CONNECTED' | 'OPTIONAL_CLOUD' | 'REJECTED';
export type ToolCapability = 'navigation' | 'interaction' | 'data' | 'system' | 'communication' | 'image' | 'vision' | 'video' | 'speech' | 'document' | 'contract';
export type NetworkPolicy = 'offline' | 'local_network' | 'internet_required' | 'optional';
export type VerificationStrategy = 'none' | 'tool_result' | 'observation' | 'independent_check';
...
export interface ExternalToolMetadata {
  ...
  privacy?: { dataTransfer: 'none'|'local_only'|'external_service'; storesInput?: boolean; storesOutput?: boolean };
  securityStatus?: 'PENDING'|'PASS'|'FAIL';
  testsStatus?: 'PENDING'|'PASS'|'FAIL';
  androidTermuxCompatible?: boolean;
  gemmaCompatible?: boolean;
  verificationStrategy: VerificationStrategy;
}

export interface Tool { id:string; name:string; description:string; inputSchema:ToolInputSchema; riskLevel:RiskLevel; category:'navigation'|'interaction'|'data'|'system'|'communication'; external?:ExternalToolMetadata; execute(params:Record<string,any>):Promise<ToolResult>; verify?(params:Record<string,any>,result:ToolResult):Promise<boolean>; isAvailable():Promise<boolean>; }
...
class ToolRegistry {
  ...
  getToolsByCapability(capability:ToolCapability){return this.getAllTools().filter(t=>t.external?.capability===capability)}
  getToolsByBackend(backend:ToolBackend){return this.getAllTools().filter(t=>t.external?.backend===backend)}
  ...
  async executeTool(id:string,params:Record<string,any>):Promise<ToolResult>{
    ...
    if(tool.riskLevel==='critical'||tool.riskLevel==='high')return this.record(requestId,id,params,{success:false,requiresConfirmation:true,confirmationMessage:this.getConfirmationMessage(tool,params),requestId,timestamp:Date.now()});
  ...
  private getConfirmationMessage(tool:Tool,params:Record<string,any>):string{const riskLabels:Record<RiskLevel,string>={low:'Low risk',medium:'Medium risk',high:'HIGH RISK',critical:'CRITICAL'};return `${riskLabels[tool.riskLevel]}: ${tool.name}\n\nThis action will be performed on your Android device.\nParameters: ${JSON.stringify(params,null,2)}\n\nDo you confirm?`;}
