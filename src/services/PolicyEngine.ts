...
class PolicyEngine {
  ...
  private initializeDefaultRules() {
    ...
    this.rules.push({ id:'daily_limits', name:'Daily Execution Limits', description:'Enforce daily execution limits for tools', enabled:true, priority:70, applies:tool=>this.maxDailyExecutions.has(tool.id), evaluate:tool=>{ const max=this.maxDailyExecutions.get(tool.id) ?? Infinity; const count=this.dailyExecutionCount.get(tool.id) ?? 0; if(count>=max) return {decision:'deny',reason:`Daily execution limit reached for ${tool.name} (${count}/${max})`,riskLevel:tool.riskLevel,requiresConfirmation:false,auditLog:{timestamp:Date.now(),tool:tool.id,decision:'deny',reason:`Daily limit exceeded: ${count}/${max}`}}; return {decision:'allow',reason:`Within daily limit (${count}/${max})`,riskLevel:tool.riskLevel,requiresConfirmation:false,auditLog:{timestamp:Date.now(),tool:tool.id,decision:'allow',reason:`Within limit: ${count}/${max}`}}; } });
  ...
  async evaluate(tool: Tool, context: PolicyContext): Promise<PolicyResult> {
    ...
    for(const rule of sortedRules){ if(!rule.applies(tool,context)) continue; const result=rule.evaluate(tool,context); this.auditLog.push(result.auditLog); if(this.auditLog.length>1000) this.auditLog.shift(); return result; }
