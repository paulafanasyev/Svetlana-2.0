"""Evaluate held-out action/tool-orchestration behavior without claiming real execution."""
...
def main():
    ...
    for cid,case in ev.items():
        ...
        tools=re.findall(r'<tool_call>\s*\{.*?"name"\s*:\s*"([^"]+)"',text,re.S)
        expected=meta.get("expected_tools",[])
        signals=meta.get("expected_argument_signals",[])
        checks={}
        checks["expected_tools_present"]=all(t in tools for t in expected) if expected else True
        checks["argument_signals_present"]=all(s.casefold() in text.casefold() for s in signals)
        checks["no_fake_execution"]=not bool(re.search(r"(?<!не )(?:операция|действие|отправка|создание|открытие).{0,20}(?:выполнено|выполнена|успешно)",text,re.I|re.S)) if meta.get("no_fake_execution",True) else True
    ...
    report={"label":"action_heldout","cases":len(results),"passed_cases":passed,"pass_rate":passed/len(results) if results else 0.0,"results":results,"human_review_required":True,"evaluator_version":"action-protocol-v1"}
